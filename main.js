...
function camera () {
    !function(){"use strict";let e=[],t=0,a=null,n=null,c=!1;function i(){c=!1,a&&clearInterval(a)}!function r(){let o=setInterval(()=>{geofs?.camera?.modes&&geofs?.aircraft?.instance&&(clearInterval(o),n=geofs.aircraft.instance.id,setInterval(()=>{if(geofs.aircraft&&geofs.aircraft.instance){let e=geofs.aircraft.instance.id;e!==n&&(n=e,console.log("Stopped cycling due to aircraft change"),i())}},1e3),document.addEventListener("keypress",n=>{"w"!==n.key.toLowerCase()||((c=!c)?(ui.notification.show("Camera cycling started."),console.log("Camera cycling started."),!geofs.camera||!geofs.camera.modes||(a&&clearInterval(a),function a(){let n=[2,3,4,5];e=geofs.camera.modes.map((e,t)=>t).filter(e=>!n.includes(e));for(let c=e.length-1;c>0;c--){let i=Math.floor(Math.random()*(c+1));[e[c],e[i]]=[e[i],e[c]]}t=0}(),a=setInterval(()=>{!geofs.pause&&c&&e.length>0&&(geofs.camera.set(e[t]),console.log("Switched to camera:",e[t]),t=(t+1)%e.length)},3e4))):(i(),ui.notification.show("Camera cycling stopped."),console.log("Camera cycling stopped.")))}),console.log("Script running. Press 'W' to toggle."))},500)}()}();
};
