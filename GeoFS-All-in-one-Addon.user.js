...
// @version      2025-03-28
// @description  25 addons, their instructions, and detailed flight procedures, compiled together
// @author       geofspilot
...
// @grant        GM_addStyle
// ==/UserScript==
let pushToTalk = 'none'; //Use this to set which version of the AI ATC addon you want. 'none' is the default version, 'text' opens the text ATC box when the PTT key is pressed, and 'voice' allows you to transmit a voice message when the PTT key is pressed.
function ai () {
        ...
        if (menuList && !menuList.querySelector('.geofs-atc-icon')) {
            ...
            document.addEventListener("keydown", function (event) {
                if (event.key.toLowerCase() === "d"  && !event.ctrlKey && !event.altKey && !event.metaKey) {
                    ...
                    const syntheticEvent = new MouseEvent('click', {
                        //cancelable: true,
                        ctrlKey: false, // Simulate Ctrl key not being pressed
                    });
